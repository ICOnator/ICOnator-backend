...
public class FileQueueService {
    ...
    private void createQueueFile() {
        try {
            ...
            throw new RuntimeException("Error on reading the email queue file.");
        }
    }
    ...
    public void addConfirmationEmail(Investor investor, URI confirmationEmaiLink) {
        Email email = new Email(MailType.CONFIRMATION_EMAIL, investor, confirmationEmaiLink);
        addEmail(email);
    }

    public void addSummaryEmail(Investor investor) {
        Email summaryEmail = new Email(MailType.SUMMARY_EMAIL, investor);
        addEmail(summaryEmail);
    }

    public Optional<Email> peekEmail() {
        Email email = null;
        try {
            byte[] obj = getQueueFile().peek();
            email = getObjectMapper().reader().forType(Email.class).readValue(obj);
        } catch (IOException e) {
    ...
    private void addEmail(Email email) {
        try {
            ...
            LOG.error("Not possible to convert Email object to string/bytes. email="
